...
| Pattern Name | Type | When and Why to Use | Status |
| :--- | :--- | :--- | :--- |
...
| **Adapter** | Structural | Use to **convert an incompatible interface** (like a third-party library) into one your client code expects. | Covered in embedded track (HardwareAdapter -- HardwareAccess domain) |
...
| **Proxy** | Structural | Use to **manage the life cycle and access** of an object. It provides the same interface as the original but can add logic like caching or sequencing. | Covered in embedded track (HardwareProxy -- HardwareAccess domain) |
...
| **Mediator** | Behavioral | Use to **centralize communication** between many nodes (like a chat server). It reduces dependencies because objects don't need to know about each other directly. | Covered in embedded track (Mediator -- HardwareAccess domain) |
...
| **Observer** | Behavioral | Use to **notify multiple subscribers** when an event occurs. This avoids wasteful "polling" where a user constantly asks a background process for a result. | Covered in embedded track (Observer -- HardwareAccess domain) |
| **State** | Behavioral | Use to implement a **state machine** where an object's behavior changes completely based on its internal state (e.g., Draft vs. Published). | Covered in embedded track (State Pattern -- StateMachines domain) |
